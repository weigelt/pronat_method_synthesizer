...
public class FunctionNameMapper {
	...
	private boolean useUnlemmatizedAlso = false;

	public FunctionNameMapper(boolean useContext, boolean usePermutations, boolean alsoMatchUnlemmatized) {
		useSynonyms = useContext;
		useUnlemmatizedAlso = alsoMatchUnlemmatized;
	...
	/**
	 * Find possible function name candidates for the extracted instruction matching
	 * in the connected ontology.
	 * 
	 * @param instruction
	 *            extracted instruction
	 * @return list of possible matching ontology methods, each ImmutablePaired with
	 *         a double similarity score
	 */
	protected List<FunctionNameCandidate> findFunctionNameCandidates(AbstractInstruction instruction) {
		...
		List<String> combinedNamesUnlematized = null;

		if (useUnlemmatizedAlso) {
			combinedNamesUnlematized = concatFunctionWithParameterNames(instruction.getInstructionName(), parameterNames);
		}
		...
		if (useSynonyms) {
			...
			for (ImmutablePair<Double, IMethod> synonymMatch : synonymMatches) {
				...
				nameCandidates.add(
						new FunctionNameCandidate((SYNONYM_NAME_WEIGHT * synonymMatch.getKey()), synonymMatch.getValue(), instruction));
			}
		}
	...
	/**
	 * Preprocess the extracted instructions. Remove stopwords and lemmatize
	 * function names.
	 * 
	...
	protected <T extends AbstractInstruction> List<T> preprocessFunctionName(List<T> instructionList) {
		...
		for (T instruction : instructionList) {
			...
			if (!clearedDscI.getClearedInstructionNameNodes().isEmpty()) {
				clearedInstructions.add(clearedDscI);
			}
		}

		logger.debug("Preprocessing the function names lead from {} to {} remaining instructions.", instructionList.size(),
				clearedInstructions.size());
		return clearedInstructions;
	}

	/**
	 * Concatenates the function name with each one of the parameters. Ensures that
	 * function name is always on first place in the String.
	 * 
	 * @param functionName
	 *            name of the instruction
	 * @param parameterNames
	 *            list of parameters of this instruction
	...
	/**
	 * Lemmatize the name (postag: verb) of the instruction and remove possible
	 * stopwords. Save it in the datastructure
	 * 
	 * @param instruction
	 *            current instruction
	...
	private <T extends AbstractInstruction> T getLemmasAndRemoveStopwords(T instruction) {
		...
		logger.debug("Cleared method name '{}' to '{}'.", instruction.getInstructionName(),
				instruction.getClearedLemmatizedInstructionName());
		return instruction;
	}
	 ...
	 *
	 * @param clearedInstructionNameNodes
	 *            instruction nodes (verbs)
	...
	private List<ImmutablePair<Double, IMethod>> getPerfectSynonymMatches(List<INode> clearedInstructionNameNodes) {
		...
		List<ImmutablePair<Double, IMethod>> matches = stringMatcher.getMethodEnsembleMatches(synonyms, null);
	...
	/**
	 * Extract wordnet synonyms of the verb(s) of one instruction. Use the synonyms
	 * from the {@link edu.kit.ipd.parse.contextanalyzer.ContextAnalyzer}.
	 *
	 * @param nodes
	 *            instruction nodes (verbs)
	...
	private List<String> getNameSynonyms(List<INode> nodes) {
		...
		for (INode node : nodes) {
			List<? extends IArc> arcs = node.getIncomingArcs().stream()
					.filter(arc -> arc.getType().equals(GraphUtils.getReferenceArcType())).collect(Collectors.toList());
