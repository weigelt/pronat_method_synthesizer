...
public class FunctionParameterMapper {
    ...
    public FunctionParameterMapper(boolean useContext) {
        ...
        parameterPosTags = parameterPosTags.length == 0 ? new String[]{defaultPosTag} : Arrays.stream(parameterPosTags).map(String::trim).toArray(String[]::new);
    ...
    /**
     * Find possible function parameter candidates for the extracted instruction matching in the connected ontology.
     * @param instructionParams extracted parameters
     * @return list of possible matching ontology parameters, each given with a double similarity score
     */
    protected <U extends AbstractInstructionParameter> List<List<FunctionParameterCandidate>> findFunctionParameterCandidates(List<U> instructionParams) {
        ...
        for (U param : instructionParams) {
            ...
            if (useSynonyms) {
                ...
                for (ImmutablePair<Double, IIndividual> synonymMatch : synonymMatches) {
                    ...
                    parameterCandidates.add(new FunctionParameterCandidate((SYNONYM_NAME_WEIGHT * synonymMatch.getKey()), synonymMatch.getValue(), param));
                }
            }

            parameterCandidateList.add(parameterCandidates);

            logger.debug("Found {} function parameter candidates for instruction parameter '{}'.", parameterCandidates.size(), param.toString());
        }
    ...
    /**
     * Preprocess the extracted instruction parameters.
     * Filter relevant part of speech words and remove stopwords.
    ...
    public <T extends AbstractInstruction<U>, U extends AbstractInstructionParameter> void preprocessFunctionParameters(T instruction) {
        ...
        for (U param : instructionParameters) {
            ...
            if (!clearedDscP.getClearedParameterNodes().isEmpty()) clearedInstructionParams.add(clearedDscP);
        }

        instruction.setClearedInstructionParameters(clearedInstructionParams);
        logger.debug("Preprocessing the function parameters lead from {} to {} remaining parameters.", instructionParameters.size(), clearedInstructionParams.size());
    }

    /**
     * Whitelist relevant part of speech tags for instruction parameters and remove stopwords.
     *
     * @param instructionParam current parameters
    ...
    private <U extends AbstractInstructionParameter> U getCorefAndFilterPosAndRemoveStopwords(U instructionParam) {
        ...
        logger.debug("Cleared parameter name '{}' to '{}'.", instructionParam.getParameterName(), instructionParam.getClearedNominalizedParameterName());
        return instructionParam;
    }

    /**
     * The parameter candidates of multiple DeclarationInstructions (for building the method signature)
     * are handled as synonyms. Therefore only new, unseen candidates should be added to the list.
     *
     * @param mainList already detected function parameter candidates
     * @param synonymList new detected function parameter candidates
    ...
    public List<List<FunctionParameterCandidate>> addWithoutDuplicates(List<List<FunctionParameterCandidate>> mainList,
        ...
        for (FunctionParameterCandidate synonym : synonymList.stream().flatMap(List::stream).collect(Collectors.toList())) {
            ...
            if (mainList.stream().flatMap(List::stream).anyMatch(c -> c.getParameterCandidate().equals(synonym.getParameterCandidate()))) continue;
            newItems.add(synonym);
        }
        if (!newItems.isEmpty()) mainList.add(newItems);
    ...
    /**
     * Search for perfect matches of function parameter synonyms in the ontology methods.
     *
     * @param clearedParameterNodes instruction nodes (nouns)
    ...
    /**
     * Extract wordnet synonyms of the noun parameters of one instruction.
     * Use the synonyms from the {@link edu.kit.ipd.parse.contextanalyzer.ContextAnalyzer}.
     *
     * @param nodes instruction parameter nodes (nouns)
    ...
    /**
     * Extract coreference information of the noun parameters.
     * Use the contextRelation tags from the {@link edu.kit.ipd.parse.contextanalyzer.ContextAnalyzer}.
     * Only use the pronominal anaphora and object resolution.
     *
     * @param node instruction parameter node (nouns)
     * @return list of coref nouns  (e.g. "it" -> "the table")
     */
    private String getParameterCoreference(INode node) {
        String coreference = "";

        if (node.getAttributeValue("pos").equals("DT")) return coreference; // skip these
        ...
        if (referenceNode == null) return coreference;
        ...
        if (!coreference.equals("")) logger.debug("Found coreference '{}' of function parameter '{}'.",  coreference, node.getAttributeValue("value"));
        return coreference;
    }
}
