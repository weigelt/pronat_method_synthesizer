...
public class SrlExtractor {
    ...
    /**
     * Get V-SRL-nodes of each chunk. Could consist of main V-SRL and modifier V-SRL nodes ("need to go", "turn to face").
     * @param nodes of utterance
     * @return Map of main V-SRL-node with itself + possible modifiers in List; Main-V-SRL-node is always first in list!
     */
     ...
     * @param srlMainVNodes main V-SRL nodes (from getMainAndModifierVSrlNodes.keySet()!)
    ...
    public List<List<INode>> getSrlChunksFromMainVSrlNodes(List<INode> srlMainVNodes) {
        ...
        for (INode node : srlMainVNodes) {
            ...
            logger.debug("Got SRL-chunks of SRL-V-token '{}': {}", node.getAttributeValue("value"), GraphUtils.getUtteranceString(sortedSrlChunkSet));
    ...
    private INode getMainVNode(INode vNode1, INode vNode2) {
        ...
        logger.debug("Found main SRL-V node {}. Skip the modifying/ first SRL-V node {}.",
                vNode1.getAttributeValue("value"), vNode2.getAttributeValue("value"));
        return mainVerbNode;
    }


    /**
     * Recursively find annotated SRL-nodes: Follow OUTgoing-SRL-arcs from SRL-V node.
     * @param node  node to follow its SRL-arcs
     * @param srlChunkSet connected SRL-nodes
...
//        logger.debug("New recursive loop. Found SRL-tokens: {}", GraphUtils.getUtteranceString(new ArrayList<>(srlChunkSet)));
        ...
        for (IArc arc : srlArcs) {
            if (!arc.getAttributeValue("role").equals(VRole)) {
                ...
                if (srlChunkSet.contains(nextSrlNode)) return srlChunkSet;
    ...
    public List<List<INode>> findParameterNodes(List<INode> nodes) {
        ...
        for (INode node : nodes) {
            ...
            if (!srlArcs.isEmpty()) {
                ...
                // skip (modal) verbs
                if (srlArcs.stream().anyMatch(arc -> arc.getAttributeValue("role").equals(ModalSrlRole))) continue;
                if (srlArcs.stream().anyMatch(arc -> arc.getAttributeValue("role").equals(VRole))) continue;
...
//                    logger.debug("Found token: '{}' with role: '{}'", role, node.getAttributeValue("value"));
                    ...
                    List<INode> argChunk;
                    List<INode> flatChunkList = roles.values().stream()
                            .flatMap(List::stream).collect(Collectors.toList());
                    if (roles.containsKey(role)) {
                        if (flatChunkList.contains(node)) continue;     // node already in list
