...
public class StringOntologyMatcher {
    ...
    /**
     * Find matches between the given method names and the elements of the connected ontology.
     * Use an ensemble of two string distance metrics: Jaro Winkler and Fuzzy Search.
     *
     * @param combinedMethodName list of extracted method names to search for
     * @return list of possible method matches, each ImmutablePaired with a double similarity score
     */
    public List<ImmutablePair<Double, IMethod>> getMethodEnsembleMatches(List<String> combinedMethodName) {
        ...
        for (String name : combinedMethodName) {
    ...
    /**
     * Find matches between the given parameter names and the elements of the connected ontology.
     * Use an ensemble of two string distance metrics: Jaro Winkler and Fuzzy Search.
     * @param permutedParameterName list of extracted parameter names to search for
     * @return list of possible parameter matches, each ImmutablePaired with a double similarity score
     */
    ...
    private List<ImmutablePair<Double, IIndividual>> getParameterMatchesByStrategy(String name, ISearchStrategy strategy) {
        ...
        for (ImmutablePair<Double, IValue> member : memberValues) {
            if (types.contains(member.getRight().getName())) continue; // skip typedobject as IValue e.g. (Drinkable)
    ...
    /**
     * Get a map of ontology item matches with their similarity score.
     * Only save the best score for each unique item.
     *
    ...
    /**
     * Find overlap of method matches between the list of found matches of the two distance metrics.
     * @param jw jaro winkler method matches
     * @param fs fuzzy search method matches
     * @return List of overlapping method matches, each ImmutablePaired with the double similarity score
     */
    ...
    /**
     * Find overlap of parameter matches between the list of found matches of the two distance metrics.
     * @param jw jaro winkler parameter matches
     * @param fs fuzzy search parameter matches
     * @return List of overlapping parameter matches, each ImmutablePaired with the double similarity score
     */
    private List<ImmutablePair<Double, IIndividual>> getOverlappingParamMatches(Map<IIndividual, Double> jw, Map<IIndividual, Double> fs) {
        ...
        for (IIndividual overlap : jw.keySet()) {
            ...
            if (fs.containsKey(overlap)) {
                ...
                if (avgScore <= 0.4) logger.debug("drop overlap match {} with avg score {} <= 0.4)", overlap.getName(), avgScore);
            }
        }
    ...
    public IObject checkForTypedObjects(String name, IDataType type) {
        ...
        if (!typedObjects.getTypes().contains(type.getName())) return null;
    ...
    /**
     * Get suffix of ontology function name and try to (sub)string match them with the
     * given parameter name.
     * e.g. lookAt -> At, lowerHead -> Head, liftRightHand -> RightHand
     *
     * @param ontologyFunctionName function name
        ...
        if (indexWordsAfterFunctionVerb > -1) {
            ...
            for (String functionNameSuffix : ontologyFunctionName.split("_")) {
                ...
                if (parameterName.toLowerCase().contains(functionNameSuffix)) {
                    logger.debug("Found substring ontology method '{}' in instruction parameter / parameter candidate '{}'", functionNameSuffix, parameterName);
                    return true;
                }
            }
        }
